...
int main()
{
    ...
    while (1)
    {
        ...
        {
            TempStatus = 0;
            printf("Temperature too HIGH(OVER 30 Degree)!\n");
...
void SetUp()
{
    ...
    uint8_t ThresholdTemp[2] = {29, 30};
    TWIM_ftm(Mode, SLAADD, ConfigRegistor, 0x01, 0, &zero, WAITTICK);        //設定連續感測模式
    TWIM_ftm(Mode, SLAADD, ConfigRegistor, 0x02, 1, &zero, WAITTICK);        //設定溫度比較模式
    ...
    TWIM_trm(Mode, SLAADD, T_low_Registor, 2, &ThresholdTemp[0], WAITTICK);  //設定低溫門檻為29度
    TWIM_trm(Mode, SLAADD, T_high_Registor, 2, &ThresholdTemp[1], WAITTICK); //設定高溫門檻為30度
}
